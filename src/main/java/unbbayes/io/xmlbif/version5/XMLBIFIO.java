...
import unbbayes.io.xmlbif.version5.xmlclasses.ObjectFactory;
import unbbayes.io.xmlbif.version5.xmlclasses.XMLBIF;
import unbbayes.io.xmlbif.version5.xmlclasses.XMLBIFType;
...
public class XMLBIFIO {
	...
	public static void loadXML(File input, SingleEntityNetwork pn) throws LoadException, IOException, JAXBException{
		...
		JAXBContext context = JAXBContext.newInstance("unbbayes.io.xmlbif.version5.xmlclasses");    	
			...
			for(int i = 0; i < distributionList.size(); i++){
				...
				if (parents != null) {
					...
					for (int j = 0; j < parentList.size(); j++) {
						...
						// Force the change in the parent list order to make sure the values in the distribution will be correct.
						childNode.getParents().set(j, pn.getNode(parent.getName()));
					}
				}
